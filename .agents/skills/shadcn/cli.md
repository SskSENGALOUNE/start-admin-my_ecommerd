...
**Admin ใน monorepo นี้:** คอมโพเนนต์ shadcn ถูก consume ผ่าน **`@devhop/ui`** — ห้ามใช้ `add` เป็นวิธี “ติดตั้ง” ลง source แอป ส่วนคำสั่ง CLI ใช้สำหรับ **docs / search / view** หรืออ้างอิง flow โปรเจกต์อื่นที่ยัง copy ไฟล์ shadcn ใน repo

Configuration is read from `components.json` (Admin อาจไม่มีไฟล์นี้).
